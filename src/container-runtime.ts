 ...
 * Apple Container: uses the bridge gateway IP (192.168.64.1) since
 *   host.docker.internal is not supported.
...
function detectHostGateway(): string {
  if (CONTAINER_RUNTIME_BIN === 'container') {
    ...
    // Fallback: standard Apple Container gateway
    return '192.168.64.1';
  }
 ...
 * Apple Container (macOS): bind to 0.0.0.0 — containers reach the host via bridge IP.
...
function detectProxyBindHost(): string {
  if (os.platform() === 'darwin') {
    // Apple Container: containers reach host via bridge, so proxy must listen broadly
    if (CONTAINER_RUNTIME_BIN === 'container') return '0.0.0.0';
    // Docker Desktop: host.docker.internal routes to loopback
    return '127.0.0.1';
  }
